...
function filterCircles(province, party, voteMargin, ru_party, voteTurnout, na_range) {
  ...
  function filterRUParty(datum) {
    if (ru_party == null) {
      ...
      if (ru_party.includes("Rest")) {
        ru_party = ru_party.filter(function (d) {
          return d != "Rest";
        });
  ...
  function compFilter(filtered) {
    return function (d) {
      ...
      filt_party && filt_VM && filt_party_ru && filterVoteTurnout(d["Percentage of Votes Polled to Registered Voters"]) && filterNARange(+d.seat.replace("NA-", ""));
      if (filtered == true) {
        return logical;
      } else {
        return !logical;
  ...
  var num_filtered = d3.selectAll(".naSeatCircle").filter(compFilter(true)).data().length;
  ...
  var polygon = d3.selectAll(".clip").data(voronoi.polygons(nodes_filtered), function (d) {
    return d.data.seat;
  ...
  var polygon_enter = d3.selectAll(".clip").enter().append("clipPath").attr("class", "clip NAmap")
  //Make sure each clipPath will have a unique id (connected to the circle element)
  .attr("id", function (d) {
    return d != null ? "clipNAmap" + d.data.seat : "clipNAmap" + "NA";
  })
  ...
  var polygon_exit = d3.selectAll(".clip").exit().remove();
...
function filterCirclesPr(province, party, voteMargin, ru_party, voteTurnout, na_range) {
  ...
  var num_filtered = d3.selectAll(".pSeatCircle").filter(compFilter(true)).data().length;

  //console.log(num_filtered);

  // filtered remain
  d3.selectAll(".pSeatCircle").filter(compFilter(true)).transition().duration(filtTransTime).attr('r', function (d) {
    return d.radiusInit;
  });
  // unfiltered fadeout
  d3.selectAll(".pSeatCircle").filter(compFilter(false)).transition().duration(filtTransTime).attr('r', 0);

  ///// transition for the circle catchers /////

  // filtered circle catchers remain
  d3.selectAll(".circle-catcher").filter(compFilter(true)).transition("cicleCatchTrans").duration(filtTransTime).attr('r', function (d) {
    return circle_catch_rad_prov;
  }).style('display', 'block');
  // unfilteres circle catchers fadeout
  d3.selectAll(".circle-catcher").filter(compFilter(false)).transition("cicleCatchTrans").duration(filtTransTime).attr('r', 0).style('display', 'none');
  ...
  //Make sure each clipPath will have a unique id (connected to the circle element)
  .attr("id", function (d) {
    return d != null ? "clip" + d.data.seat : "clipPMap";
  }).select("path").attr("class", "clip-path-circle NAmap").call(redrawPolygon);
  // enter selection, create new clipPaths
  var polygon_enter = d3.selectAll(".clip").enter().append("clipPath").attr("class", "clip NAmap")
  //Make sure each clipPath will have a unique id (connected to the circle element)
