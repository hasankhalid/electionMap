...
function makeSummBar(result) {
  ...
  d3.select('#majorityVote').append('div').style('display', 'flex').style('justify-content', 'center').style('margin-bottom', '10px').append('div').style('display', 'flex').attr('id', 'majorityInfo');

  d3.select('#majorityInfo').append('div').attr('id', 'infoPict').html(image(majority_party));

  d3.select('#majorityInfo').append('div').attr('id', 'infoText').html(function () {
    return '<p>' + majority_party + '</p><p style="margin: 0px">' + majority_seats + ' seats won</p>';
  });

  var summBarG = d3.select("#barcontain").append("svg").attr("width", width).attr("height", height).attr('id', 'barsvg').attr("preserveAspectRatio", "none");

  var max_seats = d3.max(stack(result).map(function (d) {
    return d[0][1];
  }));
  ...
  summBarG.selectAll('rect').data(stacked_data).enter().append('rect').attr('x', function (d) {
    return seat_scale(d[0][0]) + "%";
  }).attr('y', 5).attr('width', function (d) {
    return seat_scale(d[0][1] - d[0][0]) + "%";
  }).attr('height', bar_height).style('fill', function (d) {
    return ordinal(abbreviate(d.key));
    ...
    d3.select('.chordtool').append('div').classed('cpartyicon', true).html(image(d.key));
  ...
  summBarG.selectAll("text").data(stacked_data).enter().append('text').attr('x', function (d) {
    return seat_scale(d[0][0]) + "%";
  }).attr('y', 5 + bar_height + 2 + text_size).style('font-size', text_size + "px").text(function (d) {
    return (d[0][1] - d[0][0]) / max_seats > 0.03 ? d[0][1] - d[0][0] : "";
  });
}
